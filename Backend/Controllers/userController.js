...
const getProfile = async (req, res) => {
    try {
        const user = await User.findById(req.user.id).select("-password -storedOtp");
        if (!user) {
            return res.status(404).json({ message: "User not found" });
        }
...
module.exports = { registerUser, updatedUser, deletedUser, verifyOtp, loginUser, getProfile }
